...
def open_window_all_workers():
    users: list = []

    class User:
        def __init__(self, worker_name, worker_surname, location, type_of_duty):
    ...
    def add_users():
        worker_name = entry_marina_name.get()
        worker_surname = entry_owner_surname.get()
        type_of_duty = entry_workers.get()
        location = entry_location.get()
        if not (worker_name and worker_surname and type_of_duty and location):
            show_error_popup()
            return
        try:
            tmp_user = User(worker_name=worker_name, worker_surname=worker_surname, location=location, type_of_duty=type_of_duty)
    ...
    def show_users():
        ...
        for idx, user in enumerate(users):
            listbox_lista_obiektow.insert(idx, f"{idx+1}. {user.worker_name} {user.worker_surname} {user.location} {user.type_of_duty} ")
    ...
    def user_details():
        idx = listbox_lista_obiektow.index(ACTIVE)
        label_name_szczegoły_obiektu_wartosc.configure(text=users[idx].worker_name)
    ...
    def edit_user():
        idx = listbox_lista_obiektow.index(ACTIVE)
        entry_marina_name.insert(0, users[idx].worker_name)
        entry_owner_surname.insert(0, users[idx].worker_surname)
        entry_location.insert(0, users[idx].location)
        entry_workers.insert(0, users[idx].type_of_duty)
    ...
    def update_users(idx):
        ...
        try:
            ...
            pass
        users[idx].worker_name = worker_name
        users[idx].worker_surname = worker_surname
        users[idx].location = location
        users[idx].type_of_duty = type_of_duty
