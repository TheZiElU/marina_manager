...
def open_window01():
    ...
    def user_details():
        ...
        label_surname_szczegoły_obiektow_wartosc.configure(text=users[idx].worker_surname)
        label_location_szczegoły_obiektu_wartosc.configure(text=users[idx].location)
        label_posts_szczegoły_obiektow_wartosc.configure(text=users[idx].type_of_duty)
    ...
    # Okno
    root = Toplevel()
    root.title("Szczegóły pracowników")
